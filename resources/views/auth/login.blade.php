...
@section('content')
    <section style="padding: 80px 20px; text-align: center;">
        <h1 style="font-size: 48px; color: #ff4655; text-shadow: 2px 2px 5px rgba(0, 0, 0, 0.5);">Вход</h1>
        @if ($errors->any())
        ...
        @endif
        <form method="POST" action="{{ route('login') }}" style="display: flex; flex-direction: column; max-width: 400px; margin: 0 auto; gap: 15px;">
            @csrf
            ...
            <button type="submit" style="padding: 10px 20px; background: #ff4655; color: #fff; border: none; border-radius: 5px; font-size: 18px; cursor: pointer;">Войти</button>
        </form>
    </section>
@endsection